...
class OrganizacaoApi {
    ...
    async criar(req, res) {
        const { organizacao, cnpj, telefone, email } = req.body

        try {
            const response = await OrganizacaoController.criar(organizacao,cnpj,telefone,email)
    ...
    async editar(req, res) {
        const { organizacao, cnpj, telefone, email, senha, ieSituacao } = req.body
        const { idOrganizacao } = req.params

        try {
            const response = await OrganizacaoController.editar(idOrganizacao, organizacao, cnpj, telefone, email, senha, ieSituacao)
    ...
    async login(req, res) {
        ...
        try {
            const response = await OrganizacaoController.login(cnpj, senha)
            return res.status(200).send(response)
        } catch (e) {
            return res.status(400).send({ error: e.message })
        }
